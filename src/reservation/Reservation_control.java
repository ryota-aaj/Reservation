...
public String FaciExplanation(String facility){
	String res="";
	...
	if (flagLogin){
	 switch(facility){
	  ...
	  case "大会議室1":
	   res= "大会議室1:　一階の大きな会議室です。\n　　　　　　給水機が設置されています。\n";
	   break;
	
	  case "大会議室2":
	   res=	"大会議室2：　二階の大きな会議室です。\n　　　　　　給水機が設置されています。\n";
	   break;
	   
	 case"小会議室1":
	  res= "小会議室1：　一階の小さな会議室です。\n　　　　　　喫煙室が近くにあります。\n";
	  break;
	 
	 case"小会議室2":
	  res= "小会議室2：　二階の小さな会議室です。\n　　　　　　プロジェクターが設置されています。\n";
	  break;
	 
	 case"小会議室3":
	  res= "小会議室3：　三階の小さな会議室です。\n　　　　　　机、椅子ともに新品です。\n";
	  break;
	 
	 case"小会議室4": 
	  res= "小会議室4：　四階の小さな会議室です。\n　　　　　　喫煙室が近くにあります。";
	  break;

     case"小会議室5":
	  res= "小会議室5：　五階の小さな会議室です。\n　　　　　　プロジェクターが設置されています\n";
	  break;
	 
     case"小会議室6":
	  res= "小会議室6：　六階の小さな会議室です。\n　　　　　　見晴らしがとてもいいです。\n";
      break;
}
...
public String CancelReservation(Reservation_view frame){
	...
	if (start.compareTo(st)==0 && et.compareTo(end)==0){  //レコードの開始時刻=新規の開始時刻　AND　新規の開始時刻=レコードの
	...
	private boolean cancelReservationDate( int y, int m, int d){
		...
		//予約の状況
		public String CheckReservation( String facility){
		 String res = "";
		...
		try {
			// クエリーを実行して結果セットを取得
		ResultSet rs = mysql.getReservationTo(facility); // 検索結果から予約状況を作成
		 boolean exist = false;
		 while(rs.next()){
